...
const middleware = (req, res, next) => {
    ...
    //console.log("APPSERV UP2ID CONNECT",req.body)
    next()
}
...
const protectPROXY = async (req, res, next) => {
    ...
    if (req.headers["authorization"]) {
        ...
        try {
            ...
            //console.log("UP2IDCONNECT KEY", key)
            let decode = await verifyAppToken(token, key)
            //console.log("UP2IDCONNECT KEY", decode)
            next()
        } catch (error) {
            let err = {}
            err.status = 500
...
const authenticate = (req,res) => {
    ...
    //console.log("****REQ URI INSERT****", reqUri)
...
const verify = (req, res, next) => {
    authManager.token.getToken(req.originalUrl)
        .then(async function (user) {
            //console.log('******* DATA FROM AUTH SERVER')
            ...
            try {
                ...
                //console.log("****REQ URI GETBACK****", retUri)
