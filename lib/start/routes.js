...
/*const onProxyResErrSSE = responseInterceptor(async (responseBuffer, proxyRes, req, res) => {
    try {
        const buffer = responseBuffer.toString('utf8')
        let response = JSON.parse(responseBuffer.toString('utf8'));
        ...
        if (response.status >= 300) {
            ...
            if (sse && process.env.SSE == 'ON')
                sse.send(JSON.stringify(response), "message")
        }
        return JSON.stringify(response)
    } catch (error) {
        const err = {
        ...
        for (let i = 0; i < routes.length; i++) {
            ...
            if (routes[i].loginproxy) {
                ...
                if (process.env.DEBUG_RESPONSE)
                    ...
                    routes[i].proxy.onProxyReq = fixRequestBody
                //routes[i].proxy.onProxyRes = onProxyResErrSSE
            }
